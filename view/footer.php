
<div class="title-bst">
        <h2>CANIFALIFE</h2>
        <div class="online">
          <button>Xem thêm</button><i class="fas fa-angle-double-right"></i>
        </div>
      </div>
      <div class="canifalife">
        <div class="canifalife-item">
          <a href="">
            <img src="img/game-online-1.webp" alt="" />
          </a>
          <span>ĐỒ ĐẸP, GIÁ HỜI, SẮM SỬA THẢNH THƠI CÙNG SIÊU HỘI VOUCHER NHÀ
            CANIFA
          </span>
          <p class="real-time">20:31 24/10/2023</p>
        </div>
        <div class="canifalife-item">
          <a href="">
            <img src="img/game-online-2.webp" alt="" />
          </a>
          <span>ĐỒ ĐẸP, GIÁ HỜI, SẮM SỬA THẢNH THƠI CÙNG SIÊU HỘI VOUCHER NHÀ
            CANIFA
          </span>
          <p class="real-time">20:31 24/10/2023</p>
        </div>
        <div class="canifalife-item">
          <a href="">
            <img src="img/game-online-3.webp" alt="" />
          </a>
          <span>ĐỒ ĐẸP, GIÁ HỜI, SẮM SỬA THẢNH THƠI CÙNG SIÊU HỘI VOUCHER NHÀ
            CANIFA
          </span>
          <p class="real-time">20:31 24/10/2023</p>
        </div>
        <div class="canifalife-item">
          <a href="">
            <img src="img/game-online-3.webp" alt="" />
          </a>
          <span>ĐỒ ĐẸP, GIÁ HỜI, SẮM SỬA THẢNH THƠI CÙNG SIÊU HỘI VOUCHER NHÀ
            CANIFA
          </span>
          <p class="real-time">20:31 24/10/2023</p>
        </div>
      </div>
      <div class="register">
        <h3 class="block-title">Đăng ký nhận bản tin</h3>
        <span>Cùng Canifa Blog cập nhật những thông tin mới nhất về thời trang và
          phong cách sống.</span>
        <div class="form-subscribe">
          <input type="text" name="" placeholder="Nhập email đăng ký của bạn" class="input-subscribe" />
          <button type="submit" title="Gửi" class="btn-subscribe">
            <span>Đăng ký</span>
          </button>
        </div>
        <div class="block-social">
          <i class="social fab fa-facebook-square"></i>
          <i class="social fab fa-instagram-square"></i>
          <i class="social fab fa-youtube-square"></i>
          <i class="social fab fa-tiktok"></i>
        </div>
      </div>
    </article>

  <footer class="site-footer">
    <div class="container footer2">
      <div class="footer-content">
        <div class="row">
          <div class="footer-colum1">
            <h3>CÔNG TY CỔ PHẦN CANIFA</h3>
            <p>
              Số ĐKKD: 0107574310, ngày cấp: 23/09/2016, Nơi cấp: Sở Kế hoạch
              và đầu tư Hà Nội
            </p>
            <p>
              Địa chỉ trụ sở tại số 688 Đường Quang Trung, Phường La Khê, Quận
              Hà Đông, Thành phố Hà Nội.
            </p>
            <p>
              Địa chỉ liên hệ: P301, tầng 3, tòa nhà GP Invest, số 170 La
              Thành, Phường Ô Chợ Dừa, Quận Đống Đa, Thành Phố Hà Nội.
            </p>
            <p>Điện thoại: +8424 - 7303.0222</p>
            <p>Fax: +8424 - 6277.6419</p>
            <p>Email: hello@canifa.com</p>
            <div class="block-social">
              <i class="social fab fa-facebook-square"></i>
              <i class="social fab fa-instagram-square"></i>
              <i class="social fab fa-youtube-square"></i>
              <i class="social fab fa-tiktok"></i>
            </div>
          </div>
          <div class="footer-colum2">
            <h3>Thương hiệu</h3>
            <ul>
              <li title="Giới thiệu">
                <span>Giới thiệu</span>
              </li>
              <li title="Tin tức">
                <span>Tin tức</span>
              </li>
              <li title="Tuyển dụng">
                <span>Tuyển dụng</span>
              </li>
              <li title="Với cộng đồng">
                <span>Với cộng đồng</span>
              </li>
              <li title="Hệ thống cửa hàng">
                <span>Hệ thống cửa hàng</span>
              </li>
              <li title="Liên hệ">
                <span>Liên hệ</span>
              </li>
            </ul>
          </div>
          <div class="footer-colum3">
            <h3>Hỗ trợ</h3>
            <ul>
              <li title="Hỏi đáp">
                <span>Hỏi đáp</span>
              </li>
              <li title="Chính sách KHTT">
                <span>Chính sách KHTT</span>
              </li>
              <li title="Điều kiện - Điều khoản Chính sách KHTT">
                <span>Điều kiện - Điều khoản Chính sách KHTT</span>
              </li>
              <li title="Chính sách vận chuyển">
                <span>Chính sách vận chuyển</span>
              </li>
              <li title="Gợi ý tìm size">
                <span>Gợi ý tìm size</span>
              </li>
              <li title="Kiểm tra đơn hàng">
                <span>Kiểm tra đơn hàng</span>
              </li>
              <li title="Tra cứu điểm thẻ">
                <span>Tra cứu điểm thẻ</span>
              </li>
              <li title="Chính sách bảo mật thông tin KH">
                <span>Chính sách bảo mật thông tin KH</span>
              </li>
            </ul>
          </div>
          <div class="footer-colum4">
            <h3>Tải ứng dụng</h3>
            <div class="application">
              <div class="bancode">
                <img src="" width="100" height="100" alt="" />
              </div>
              <div class="apps">
                <a href="" target="_blank" aria-label="Google Play"><img src="" width="100" height="100" alt="" /></a>
                <a href="" target="_blank" aria-label="Apple Store"><img src="" width="100" height="100" alt="" /></a>
              </div>
            </div>
            <div class="payment">
              <h3>Phương thức thanh toán</h3>
              <p>
                <img src="" width="100" height="100" alt="" />
              </p>
            </div>
          </div>
        </div>
        <div class="footer-bottom">
          <div class="copyright">© 2023 CANIFA</div>
          <div class="bct">
            <a href="" target="_blank" aria-label="Pay"><img src="" width="100" height="100" alt="" /></a>
          </div>
        </div>
      </div>
  </footer>
</body>
<script src="script.js"></script>
</html>